...
На macOS после клонирования можно выполнить один скрипт:

```bash
./scripts/bootstrap_agent.sh
```

Он устанавливает зафиксированные зависимости и регистрирует локальный MCP-сервер, если найден Codex CLI. Скрипт специально не создаёт профиль и не спрашивает токен: владелец сам выбирает разрешённый сервер и передаёт свой секрет только скрытому `auth set` приглашению.
